 ...
 *								dragMaxjScrollPaneContainer	-	The maximum width to allow the drag bar to be
	...
	settings = jQuery.extend(
		{
			...
			animateInterval : 50,
			animateStep: 6,
			maintainPosition: false, // per evitare casini nel ridimensionamento quando reinizializzato
			resize: true,
			minimumWidth: 200,
			reset: false
		}, settings
	...
	return this.each(
		function()
		{
			...
			if (jQuery(this).parent().is('.jScrollPaneContainer')) {
				var currentScrollPosition = settings.maintainPosition ? $this.offset().left : 0;
				...
				if((previousWindow != $(window).width()) && ((rightPos > $(window).width()) || (previousWindow < $(window).width())) && (settings.resize == true)) {
					...
					if($(window).width() >= _jscr_originalSizes[currentId]) {
						...
						//Give the outside edge a 5 px buffer margin
						paneWidth = $(window).width() - ($this.offset().left + _jscr_originalPos[currentId]) - 5;
					}
